...
open class Http: NSObject {
    ...
    public func json (_ api:String?,_ params:NSMutableDictionary?,_ method:String?,ai:Bool,popup:Bool,prnt:Bool,_ header:NSDictionary? = nil ,_ images:NSMutableArray? = nil,sync:Bool = false,defaultCalling:Bool=false,completionHandler: @escaping (Any?,NSMutableDictionary?,String, HTTPURLResponse?) -> Swift.Void) {
        ...
        if (reach?.isReachable)! {
            ...
            completionHandler (nil,params,"", nil)
        }
    }
    @objc public func jsonThread(_ md:NSMutableDictionary) {
        let completionHandler = md["completionHandler"] as? ((Any?,NSMutableDictionary?,String, HTTPURLResponse?) -> Swift.Void)
        var data:Data! = nil
        ...
        if error != nil {
            ...
            completionHandler!(nil,params,jsonString, response as? HTTPURLResponse)
        } else {
            do {
                ...
                DispatchQueue.main.async {
                    completionHandler!(parsedData,params,jsonString, response as? HTTPURLResponse)
                }
            } catch _ as NSError {
                var prnt = "====================================================================="
                if (api != nil) { prnt += "\n" + "api -\(api!)-" }
