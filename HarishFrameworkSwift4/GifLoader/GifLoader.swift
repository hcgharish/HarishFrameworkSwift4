...
public class GifLoader: NSObject {
    let lock_Queue = DispatchQueue.init(label: "com.kavya.lock_Queue.gifloader")
    ...
    public var maGif:[UIImage]? = nil
    public var gifFilename:String? = nil
    var gifImage:UIImage? = nil
    var imageView:UIImageView? = nil
    public var duration:Double = 1.0
    public func showLoader () {
        DispatchQueue.main.async {
            self.lock_Queue.sync {
                if self.noofrequest == 0 {
                    ...
                    self.container?.center = (self.appDelegate?.window??.center)!
                        ...
                        self.imageView = UIImageView(frame: CGRect(x:0,y:0,width:self.gifSize.width,height:self.gifSize.height))
                    }
                                    if self.imageView != nil {
                        if self.gifImage != nil {
